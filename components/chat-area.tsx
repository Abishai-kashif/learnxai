...
export function ChatArea({ user }: { user: User }) {
  ...
  // 👇 textarea ka ref for auto-resize
  ...
  const submitMessage = async () => {
    ...
    let _session = [...session, userMessage];
    setSession(_session);
    setInput("");
          ...
          {session.length === 0 ? (
            <div className="h-[60vh] flex flex-col items-center justify-center text-center text-muted-foreground gap-4">
              ...
              <div>
                ...
                <p className="text-sm">
                  Ask anything about learning topics, generate quizzes, or get explanations.
                </p>
              </div>
              <div className="flex flex-wrap items-center justify-center gap-2 mt-2">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => setInput(PROMPT_SUGGESTIONS.EXPLAIN_RECURSION)}
                >
                  Try: Explain recursion
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => setInput(PROMPT_SUGGESTIONS.CREATE_JS_QUIZ)}
                >
                  Create a JS quiz
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  onClick={() => setInput(PROMPT_SUGGESTIONS.SUMMARIZE_GRADIENT_DESCENT)}
                >
                  Summarize a concept
                </Button>
              </div>
            ...
            session.map((message, index) => {
              return <ChatMessage key={index} {...message} {...(message.role == "user" ? { user } : {})} />;
            })
          )}

          {currentResponse && <ChatMessage role="assistant" content={currentResponse} />}
        </div>
      </ScrollArea>
      <div />
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              rows={1}
              className="w-full px-4 py-3 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 pr-12 bg-background text-foreground resize-none overflow-hidden break-words"
            />
             <Button
    size="sm"
    onClick={submitMessage}
    disabled={loading || input.trim().length === 0}
    className="absolute right-2 bottom-3 bg-orange-500 hover:bg-orange-600 disabled:opacity-50 disabled:cursor-not-allowed"
  >
